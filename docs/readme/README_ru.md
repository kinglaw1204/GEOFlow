...
В новой пустой базе `geoflow:install` активирует Enterprise Signature 21 и импортирует 50 справочных статей. Для минимальной установки используйте `--without-demo`. Сайты с данными сохраняют тему, настройки, авторов, категории и статьи. Для ручного импорта задайте `GEOFLOW_SEED_FRONTEND_DEMO=true` и выполните `php artisan db:seed --force`; по умолчанию добавляются только отсутствующие записи. `GEOFLOW_SEED_FRONTEND_DEMO_OVERWRITE=true` предназначен для сбрасываемых демо-баз.
